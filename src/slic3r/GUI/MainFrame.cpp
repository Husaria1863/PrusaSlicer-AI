...
//        wxString dirty_marker = (!m_plater->model().objects.empty() && m_plater->is_project_dirty()) ? "*" : "";
    ...
    if (idx_plus != build_id.npos) {
        ...
        const bool unknown_suffix = boost::starts_with(build_id.data() + idx_plus + 1, "UNKNOWN");
    	if (unknown_suffix || build_id.at(idx_plus + 1) == '-' || sscanf(build_id.data() + idx_plus + 1, "%d-", &commit_after_label) == 0) {
